...
const RepairCenter = ({ type, setActiveTab }) => {
    ...
    return (
        <div className="max-w-[1600px] mx-auto space-y-8 pb-32 animate-fade-in px-4 md:px-8">
            ...
            {/* Onarım Listesi (3'lü Kompakt Yatay Görünüm) */}
            <div className="grid grid-cols-1 xl:grid-cols-2 2xl:grid-cols-3 gap-6">
                {filteredRepairs.map((repair) => (
                    <div 
                        key={repair.id} 
                        className={`group relative bg-white rounded-[28px] transition-all duration-500 flex items-stretch overflow-hidden border border-gray-100 shadow-sm hover:shadow-[0_20px_40px_-10px_rgba(0,0,0,0.08)] hover:-translate-y-1 ${type === 'apple-center' ? 'hover:border-purple-200' : 'hover:border-blue-200'}`}
                    >
                        ...
                        <div className="relative w-40 shrink-0 bg-gray-50 overflow-hidden border-r border-gray-50">
                            <img 
                                ...
                                onError={(e) => {
                                    ...
                                    // Fallback olarak ürün grubu bazlı görseli tekrar dene (belki ana resim linki bozuktur)
                                    const fallback = getProductImage(repair.productGroup, repair.device);
                                    // Eğer fallback de aynıysa (nadiren), tamamen genel bir teknoloji görseline çek
                        ...
                        <div className="p-5 flex-1 flex flex-col min-w-0 justify-between">
                            <div className="space-y-3">
                                ...
                                {type === 'apple-center' ? (
                                    <div className="grid grid-cols-1 gap-1.5">
                                        <div className="flex items-center gap-2 p-1.5 bg-purple-50 rounded-xl border border-purple-100/50">
                                            <Truck size={10} className="text-purple-600 shrink-0" />
                                            <span className="text-[9px] font-bold text-purple-900 font-mono truncate">{repair.shipmentCode || 'Takip No...'}</span>
                                        </div>
                                        <div className="flex items-center gap-2 p-1.5 bg-indigo-50 rounded-xl border border-indigo-100/50">
                                            <Zap size={10} className="text-indigo-600 shrink-0" />
                                            <span className="text-[9px] font-bold text-indigo-900 font-mono truncate">{repair.appleRepairId || 'Apple ID...'}</span>
                                        </div>
                                    </div>
                                ) : (
                            ...
                            <div className="mt-4 flex gap-2">
                                <button
                                    ...
                                    className={`flex-1 h-9 rounded-xl text-[9px] font-black transition-all shadow-md active:scale-95 flex items-center justify-center gap-2 text-white ${type === 'apple-center'
                                        ? 'bg-purple-600 hover:bg-purple-700 shadow-purple-100'
                                        : 'bg-gray-900 hover:bg-black shadow-gray-200'
                                    }`}
                                >
                                    {type === 'apple-center' ? 'YÖNET' : 'BAŞLA'} <ArrowRight size={12} strokeWidth={3} />
                                </button>
                                <button
